...
class DenseVectorNet(pl.LightningModule):
    def __init__(
        ...
        self.dense_net.fc = nn.Linear(self.dense_net.fc.in_features, num_modes * 80 * 2 + num_modes)
    ...
    def forward(
        ...
        logits, pred_trajs = (
            dense_features[:, :self.num_modes],
            dense_features[:, self.num_modes:],
        )

        pred_trajs = pred_trajs.view(-1, self.num_modes, 80, 2)
    ...
    def _training_and_validation_step(
        ...
        gt_trajs = gt_rasterized_trajs
        target_future_mask = gt_rasterized_masks
