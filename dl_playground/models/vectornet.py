...
class MLP(nn.Module):
    ...
    def forward(
        ...
        out = torch.empty(batch_size, x.shape[1], dtype=x.dtype, device=x.device)
        out[mask, :] = x
...
class SubGraph(nn.Module):
    ...
    def forward(
        ...
        x = self.MLP1(x, mask, size1, size2)
        x = self.MLP2(x, mask, size1, size2)
        x = self.MLP3(x, mask, size1, size2)
